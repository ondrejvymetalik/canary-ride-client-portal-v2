...
app.post('/api/contracts/:bookingId/create', async (req, res) => {
  try {
    ...
    console.log('📋 DEBUG: Full booking object structure:', {
    ...
    // The key insight: bookingId parameter is our booking number (like "6004")
    // but booking.id from Booqable should be the actual integer order ID
    let booqableOrderId = booking.id;
    
    // If booking.id is a UUID string, this is wrong - we need the actual Booqable order ID
    if (typeof booking.id === 'string' && booking.id.includes('-')) {
      console.log('❌ ERROR: booking.id is a UUID, this should not happen');
      console.log('🔧 Attempting to fix by fetching fresh booking data...');
      
      // The issue is that our booking number is not the same as Booqable's order ID
      // We need to find the actual Booqable order ID
      try {
        // Use the booking number to search for the order in Booqable
        const freshBooking = await booqableService.verifyBooking(bookingId.toString(), userEmail);
        if (freshBooking) {
          console.log('📋 Fresh booking structure:', {
            id: freshBooking.id,
            number: freshBooking.number,
            idType: typeof freshBooking.id,
            isStillUUID: typeof freshBooking.id === 'string' && freshBooking.id.includes('-')
          });
          
          // If the fresh booking still has a UUID, there's a deeper issue
          // Let's use the booking number as order ID since that's what Booqable likely expects
          if (typeof freshBooking.id === 'string' && freshBooking.id.includes('-')) {
            console.log('⚠️ Fresh booking also has UUID, using booking number as fallback');
            booqableOrderId = bookingId; // Use the booking number directly
          } else {
            booqableOrderId = freshBooking.id;
          }
        } else {
          throw new Error('Could not fetch fresh booking data');
        }
      } catch (fetchError) {
        console.log('❌ Failed to fetch fresh booking, using booking number as last resort');
        booqableOrderId = bookingId; // Use the booking number directly
      }
    }

    console.log('📋 Final order ID for contract creation:', {
      orderIdToUse: booqableOrderId,
      orderIdType: typeof booqableOrderId
    });

    // CRITICAL FIX: Use the booking number directly since that's what Booqable expects
    // The booking.id might be a UUID but we need the actual Booqable order ID  
    const actualOrderId = bookingId; // Use the booking number (6004) directly
    console.log('🔧 OVERRIDE: Using booking number as order ID:', actualOrderId);

    // Create contract using Booqable API
    const contract = await booqableService.createContract(actualOrderId);
